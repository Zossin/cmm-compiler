#ifndef _INTER_CODE_H_
#define _INTER_CODE_H_
...
typedef enum { FunCode, LabelDec, Assign, Add, Sub, Mul, Div, Goto, Equal, More, Less, MoreEqual, LessEqual, NotEqual, Return, Dec, Arg, Call, Param, Read, Write } code_kind;

struct InterCode_ {
    code_kind kind;
    union {
        struct { char func_name[SYMBOL_NAME_LEN]; } func;
        struct { Operand *dest; } label;
        struct { Operand *right, *left; } assign;
        struct { Operand *result, *op1, *op2; } binop;
        struct { Operand *op; } command;
        struct { Operand *result; char func_name[SYMBOL_NAME_LEN]; } call;
