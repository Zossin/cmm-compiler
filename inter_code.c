...
InterCode *gen_binop_code(code_kind kind, Operand *result, Operand *op1, Operand *op2) {
   InterCode *code = (InterCode*)malloc(sizeof(InterCode));
   code->kind = kind;
...
void print_inter_code(FILE *file, InterCodeList *codes) {
    ...
    do {
        ...
        else if (code->kind == Equal || code->kind == NotEqual || code->kind == More || code->kind == Less || code->kind == MoreEqual || code->kind == LessEqual) {
            fprintf(file, "IF ");
